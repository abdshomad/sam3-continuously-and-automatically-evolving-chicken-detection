#!/bin/bash
# Task ID: 4.2.1
# Description: Monitor Focal Loss (Presence) (Wrapper Script)
# Created: 2025-12-12

set -e

echo "=========================================="
echo "Task 4.2.1: Monitor Focal Loss (Presence)"
echo "=========================================="
echo ""
...
PYTHON_SCRIPT="$PROJECT_ROOT/scripts/task_421_monitor_focal_loss.py"
...
echo "Running focal loss monitoring script..."
echo ""

if uv run python "$PYTHON_SCRIPT" "$@"; then
    echo ""
    echo "✓ Focal loss monitoring script completed successfully"
    exit 0
else
    echo ""
    echo "ERROR: Focal loss monitoring script failed"
    exit 1
fi
