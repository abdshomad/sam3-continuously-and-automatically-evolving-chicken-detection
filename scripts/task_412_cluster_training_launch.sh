#!/bin/bash
# Task ID: 4.1.2
# Description: Cluster Training Launch (Production)
# Created: 2025-01-15

set -e

echo "=========================================="
echo "Task 4.1.2: Cluster Training Launch (Production)"
echo "=========================================="
echo ""
...
echo "Starting cluster training run..."
echo ""
...
echo "Command: $PYTHON_EXE sam3/train/train.py -c $CONFIG_ARG --use-cluster 1 --num-nodes $NUM_NODES --num-gpus $NUM_GPUS"
echo ""
echo "Note: This will submit the job to the cluster using SLURM/Submitit."
echo "      Monitor the job status using: squeue -u \$USER"
echo ""

if "$PYTHON_EXE" sam3/train/train.py -c "$CONFIG_ARG" --use-cluster 1 --num-nodes "$NUM_NODES" --num-gpus "$NUM_GPUS"; then
    echo ""
    echo "✓ Cluster training job submitted successfully"
    echo ""
    echo "The training job has been submitted to the cluster."
    echo "Monitor progress using:"
    echo "  - Job queue: squeue -u \$USER"
    echo "  - WandB dashboard: https://wandb.ai"
    echo "  - Logs: Check the experiment_log_dir specified in config"
    exit 0
else
    echo ""
    echo "ERROR: Cluster training launch failed"
    echo ""
    echo "Common issues:"
    echo "  - SLURM/Submitit not properly configured"
    echo "  - Cluster credentials not set up"
